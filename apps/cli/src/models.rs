...
pub fn download_model_sync(model_name: &str, variant: &str) -> Result<PathBuf, String> {
    ...
    let files = if variant == "int8" {
        ...
        MODEL_FILES_INT8 // Default to int8 for CLI
    };
    ...
    for (i, file) in files.iter().enumerate() {
        ...
        println!("  [{}/{}] Downloading {}...", i + 1, files.len(), file);
    ...
    let metadata = ModelMetadata {
        ...
        source: Some("istupakov/parakeet-tdt-0.6b-v3-onnx".to_string()),
...
fn download_file_sync(url: &str, dest_path: &PathBuf) -> Result<u64, String> {
    ...
    let result = Command::new("curl")
        .args(["-L", "-o", &dest_str, "--progress-bar", url])
        .status();
    ...
    Err("curl is required for downloading models".to_string())
}
